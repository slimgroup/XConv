...
class Xconv3D(torch.nn.modules.conv._ConvNd):
    def __init__(self, chi, cho, k, ps=8, bias=None, stride=1, padding=0, dilation=1, groups=1, padding_mode='zeros'):
        ...
        super(Xconv3D, self).__init__(chi, cho, kernel_size, stride, padding, dilation, False, _triple(0), groups, bias, padding_mode)
        self.ps = ps

    def forward(self, input):    
        return conv3d(input, self.weight, self.ps, self.bias, self.stride, self.padding, self.dilation, self.groups)
